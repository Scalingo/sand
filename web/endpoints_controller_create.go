...
func (c EndpointsController) Create(w http.ResponseWriter, r *http.Request, p map[string]string) error {
	...
	if params.Activate {
		...
		if params.ActivateParams.NSHandlePath == "" {
			w.WriteHeader(400)
			return errors.New(ctx, "missing ns_handle_path")
		}
		if _, err := os.Stat(params.ActivateParams.NSHandlePath); err != nil {
			w.WriteHeader(400)
			return errors.Errorf(ctx, "ns_handle_path '%s' is invalid: %v", params.ActivateParams.NSHandlePath, err)
		}
	}

	network, ok, err := c.NetworkRepository.Exists(ctx, params.NetworkID)
	if err != nil {
		return errors.Wrapf(ctx, err, "get network %v", params.NetworkID)
	}
	if !ok {
		w.WriteHeader(404)
		return errors.New(ctx, "not found")
	}
	...
	if err != nil {
		return errors.Wrapf(ctx, err, "allocate IP in pool ip=%v network=%v", params.IPv4Address, network)
	}
	params.IPv4Address = allocatedIP

	err = c.NetworkRepository.Ensure(ctx, network)
	if err != nil {
		return errors.Wrapf(ctx, err, "ensure network %s", network)
	}
	...
	if err != nil {
		return errors.Wrapf(ctx, err, "create endpoint")
	}
