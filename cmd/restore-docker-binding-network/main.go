...
func main() {
	...
	for _, endpoint := range recoveryData {
		log.Info("restoring network: ", endpoint)
		network := docker.DockerPluginNetwork{
		...
		err = dockerRepository.SaveNetwork(ctx, network)
		if err != nil {
			log.WithError(err).Error("fail to save network")
		}
	}
