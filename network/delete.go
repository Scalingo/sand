...
func (c *repository) Delete(ctx context.Context, network types.Network, a ipallocator.IPAllocator) error {
	...
	if err != nil && err != store.ErrNotFound {
		return errors.Wrapf(ctx, err, "get list of link nodes-networks of %s", network)
	}
	...
	if len(nets) == 0 {
		...
		if err != nil {
			return errors.Wrapf(ctx, err, "delete network %s from store", network)
		}

		err = a.ReleasePool(ctx, network.ID)
		if err != nil {
			return errors.Wrapf(ctx, err, "release pool of network %s", network)
		}
	} else {
