...
func (netm manager) Ensure(ctx context.Context, network types.Network) error {
	...
	if err != nil && err != netnsbuilder.ErrAlreadyExist {
		return errors.Wrapf(ctx, err, "create network namspace")
	}

	// Get a netlink handle to the root network namespace
	rootNetlinkHandle, err := netlink.NewHandle(unix.NETLINK_ROUTE)
	if err != nil {
		return errors.Wrapf(ctx, err, "could not create netlink handle on initial root namespace")
	}
	defer rootNetlinkHandle.Delete()

	err = rootNetlinkHandle.SetSocketTimeout(NetlinkSocketsTimeout)
	if err != nil {
		return errors.Wrapf(ctx, err, "configure timeout on netlink socket")
	}
	...
	if err != nil {
		return errors.Wrapf(ctx, err, "list links")
	}
	...
	if !exist {
		...
		if err != nil {
			return errors.Wrapf(ctx, err, "create bridge in namespace")
		}

		link, err = nlh.LinkByName(BridgeName)
		if err != nil {
			return errors.Wrapf(ctx, err, "get bridge link")
		}
	...
	if err != nil {
		return errors.Wrapf(ctx, err, "list addresses of %s", BridgeName)
	}

	if len(addresses) == 0 {
		brAddr, err := netlink.ParseAddr(network.Gateway)
		if err != nil {
			return errors.Wrapf(ctx, err, "parse %s IP address", network.Gateway)
		}
		err = nlh.AddrAdd(link, brAddr)
		if err != nil {
			return errors.Wrapf(ctx, err, "add %s on bridge", network.Gateway)
		}
	}
	...
	if !exist {
		...
		if err != nil {
			return errors.Wrapf(ctx, err, "error creating %s interface (VNI: %v)", vxlan.Attrs().Name, network.VxLANVNI)
		}

		link, err := rootNetlinkHandle.LinkByName(vxlan.Attrs().Name)
		if err != nil {
			return errors.Wrapf(ctx, err, "get %s link", vxlan.Attrs().Name)
		}
		...
		if err != nil {
			return errors.Wrap(ctx, err, "set netns of vxlan")
		}

		err = nlh.LinkSetName(link, VxLANInNSName)
		if err != nil {
			return errors.Wrapf(ctx, err, "rename %s to %s in ns", link.Attrs().Name, VxLANInNSName)
		}
	}
	...
	if err != nil {
		return errors.Wrapf(ctx, err, "get %s link", VxLANInNSName)
	}

	if link.Attrs().MasterIndex == 0 {
		err := nlh.LinkSetMaster(link, bridge)
		if err != nil {
			return errors.Wrapf(ctx, err, "set %s in bridge %s", VxLANInNSName, BridgeName)
		}
	}
	...
	for _, ifName := range []string{"lo", BridgeName, VxLANInNSName} {
		...
		if err != nil {
			return errors.Wrapf(ctx, err, "get %s link", ifName)
		}
		err = nlh.LinkSetUp(link)
		if err != nil {
			return errors.Wrapf(ctx, err, "set %s up", ifName)
		}
	}
	return nil
}
