...
func (m manager) EnsureEndpoint(ctx context.Context, network types.Network, endpoint types.Endpoint, params params.EndpointActivate) (types.Endpoint, error) {
	...
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "get current thread network namespace")
	}
	...
	if !params.MoveVeth {
		...
		if err != nil {
			return endpoint, errors.Wrapf(ctx, err, "get target namespace handler: %s", endpoint.TargetNetnsPath)
		}
	...
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "get host namespace handler")
	}
	defer hostnlh.Delete()

	targetnlh, err := netlink.NewHandleAt(targetnsfd, unix.NETLINK_ROUTE)
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "get target namespace netlink handler")
	}
	defer targetnlh.Delete()

	overlaynsfd, err := netns.GetFromPath(network.NSHandlePath)
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "get namespace handler %s", network.NSHandlePath)
	}
	defer overlaynsfd.Close()

	overlaynlh, err := netlink.NewHandleAt(overlaynsfd, unix.NETLINK_ROUTE)
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "get netlink handler of netns")
	}
	...
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "create veth pair")
	}

	bridge, err := overlaynlh.LinkByName("br0")
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "find br0")
	}

	err = overlaynlh.LinkSetMTU(vethOverlay, 1450)
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "set MTU 1450 on %s", vethOverlay.Attrs().Name)
	}

	err = overlaynlh.LinkSetMaster(vethOverlay, bridge)
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "add %s in bridge", vethOverlay.Attrs().Name)
	}

	err = targetnlh.LinkSetMTU(vethTarget, 1450)
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "set MTU to 1450 on %s", vethTarget.Attrs().Name)
	}

	if params.SetAddr {
		err = targetnlh.LinkSetUp(vethTarget)
		if err != nil {
			return endpoint, errors.Wrapf(ctx, err, "set link up %s in target", vethTarget.Attrs().Name)
		}

		addr, err := netutils.ParseAddr(endpoint.TargetVethIP)
		if err != nil {
			return endpoint, errors.Wrapf(ctx, err, "parse %s IP address", endpoint.TargetVethIP)
		}

		addrs, err := targetnlh.AddrList(vethTarget, nl.FAMILY_V4)
		if err != nil {
			return endpoint, errors.Wrapf(ctx, err, "list addresses of target %v", vethTarget.Attrs().Name)
		}
		...
		if !exist {
			...
			if err != nil {
				return endpoint, errors.Wrapf(ctx, err, "add %s on target veth %v", endpoint.TargetVethIP, vethTarget.Attrs().Name)
			}
		}
	}
	...
	if err != nil {
		return endpoint, errors.Wrapf(ctx, err, "set link up %s in overlay namespace", vethOverlay.Attrs().Name)
	}
...
func (e overlayEndpoint) ensureVethPair(ctx context.Context, params params.EndpointActivate) (netlink.Link, netlink.Link, error) {
	...
	if e.endpoint.OverlayVethName != "" {
		...
		if err != nil {
			return nil, nil, errors.Wrapf(ctx, err, "check existing overlay veth presence")
		}
		if ok {
			targetLink, ok, err := e.isTargetVethPresent(ctx)
			if err != nil {
				return nil, nil, errors.Wrapf(ctx, err, "check existing target veth presence")
			}
			if ok {
			...
			if err != nil {
				return nil, nil, errors.Wrapf(ctx, err, "remove overlay %s", e.endpoint.OverlayVethName)
			}
		}
	}
...
func (e overlayEndpoint) isOverlayVethPresent(ctx context.Context) (netlink.Link, bool, error) {
	...
	if _, ok := err.(netlink.LinkNotFoundError); ok {
		...
		return nil, false, errors.Wrapf(ctx, err, "get link %s", e.endpoint.OverlayVethName)
	}
...
func (e overlayEndpoint) isTargetVethPresent(ctx context.Context) (netlink.Link, bool, error) {
	...
	if err != nil {
		return nil, false, errors.Wrapf(ctx, err, "list links of target namespace")
	}
	for _, l := range links {
...
func (e overlayEndpoint) createVethPair(ctx context.Context, params params.EndpointActivate) (netlink.Link, netlink.Link, error) {
	...
	if err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "generate veth-overlay name")
	}
	vethTargetName, err := netutils.GenerateIfaceName(e.hostnlh, "sand", 4)
	if err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "generate veth-target name")
	}

	mac, err := net.ParseMAC(e.endpoint.TargetVethMAC)
	if err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "parse target MAC address")
	}
	...
	if err := e.hostnlh.LinkAdd(veth); err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "error creating veth pair")
	}

	vethOverlay, err := e.hostnlh.LinkByName(vethOverlayName)
	if err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "find %s", vethOverlayName)
	}

	err = e.hostnlh.LinkSetNsFd(vethOverlay, int(e.overlaynsfd))
	if err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "switch namespace for overlay veth")
	}

	vethTarget, err := e.hostnlh.LinkByName(vethTargetName)
	if err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "get %s", vethTargetName)
	}

	// Force interface down to avoid 'device busy' error
	err = e.hostnlh.LinkSetDown(vethTarget)
	if err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "set down target interface %v", vethTargetName)
	}

	err = e.hostnlh.LinkSetHardwareAddr(vethTarget, mac)
	if err != nil {
		return nil, nil, errors.Wrapf(ctx, err, "set mac '%s' on target interface '%v'", mac, vethTargetName)
	}

	if params.MoveVeth {
		err = e.hostnlh.LinkSetNsFd(vethTarget, int(e.targetnsfd))
		if err != nil {
			return nil, nil, errors.Wrapf(ctx, err, "switch namespace for target veth")
		}

		vethTarget, err = e.targetnlh.LinkByName(vethTargetName)
		if err != nil {
			return nil, nil, errors.Wrapf(ctx, err, "get target veth in target ns")
		}
	}
